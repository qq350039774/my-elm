<template>
    <van-list finished-text="Finished">
        <van-cell v-for="item in shopListArr" :key="item.id">
            <van-row type="flex" justify="space-between">
                ...
                <van-col span="8" align="right" style="font-size:10px;">
                    <div style="color:#A8A8A8;">
                        <!-- 保准票 -->
                        <span style="font-size:8px;padding:1px;margin-left:5px;border:1px solid rgb(226, 229, 231);border-radius: 2px;" v-for="item in item.supports" :key="item.id">{{item.icon_name}}</span>
                    </div>
                    <div>
